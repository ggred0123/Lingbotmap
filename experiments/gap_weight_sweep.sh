...
run_one(){ # $1=gpu $2=case
  ...
  local out="experiments/results/gapw_${TAG}_${scene}_t${t0}_K${K}.json"
  [ -f "$out" ] && { log "skip $scene t$t0 K$K"; return; }
  log "gpu$gpu  $scene t0=$t0 K=$K"
  CUDA_VISIBLE_DEVICES=$gpu python -u experiments/gap_weight_probe.py \
    --ckpt "$CKPT" --frames "$frames" --bank "labels/$scene" \
    --t0 "$t0" --K "$K" --gamma $GAMMA --gt_calib "$CALIB" --out "$out" \
    > "experiments/logs/gapw_${TAG}_${scene}_t${t0}_K${K}.log" 2>&1 \
