#!/usr/bin/env bash
# Keep one gtabs arm (docs/gtabs-plan.md) alive on its card until it finishes.
#
#   experiments/watchdog_gtabs.sh <arm> <gpu> [trainer flags...]
...
while :; do
  ...
  if ! alive; then
    ...
    GPU=$GPU NPROC=1 POOL=20 PROBE_MAX=1 SAVE_EVERY=$SE STEPS=$TARGET THREADS=${THREADS:-8} \
      experiments/launch_gtsup.sh "$ARM" _gt "$@" >> "$WLOG" 2>&1
  fi
  sleep "$POLL"
done
